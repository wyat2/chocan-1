        ...
        int & num_rows_in, int & num_col_in)
{
    ...
    for(int i = 0; i < num_rows; ++i)
    {
        ...
        {
            results_in[i][j] = this->results_2d[i][j];
        }
    }
}

void General_db::get_results(string results_in[MAX_RESULTS], 
        int & num_rows_in)
{
    num_rows_in = this->num_rows;
    //results_in = this->results;
...
int General_db::exec(const string sql)
{
    ...
    while(sqlite3_step(res) == SQLITE_ROW && num_rows < MAX_RESULTS)
    {
        for(int i = 0; i < num_col; ++i)
        {
            ...
            results_2d[num_rows][i] = string(reinterpret_cast<const char *>(temp));
        }
        ++this->num_rows;
    }
